...
export default function page({ params }) {
  ...
  async function fetchPartner(id) {
    try {
      const res = await fetch(`../../../api/partners/${id}`, { method: "GET" });
      const result = await res.json();

      const categoryId = result.data.partnerCategories[0]?.categoryId;
      const categoryName = result.data.partnerCategories[0]?.categories?.name;
  ...
  async function fetchCategories(categoryId) {
    try {
      const res = await fetch("../../../api/categories?filter=partner", {
        method: "GET",
      });
      const result = await res.json();
