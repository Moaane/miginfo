...
export default function Page() {
  ...
  async function fetchClients() {
    try {
      ...
      console.log(typeof result.data[0].name);
    } catch (error) {
      console.log(error);
    }
  }
          ...
          {datas.map((data) => (
            ...
            >
              <div className="flex flex-wrap justify-center md:justify-start p-4 gap-6">
                {data.clientCategories?.map((prop) => (
                  <div key={prop.clients.id}>
                    <div className="w-full px-10 md:w-40 xl:w-52 h-32 group overflow-hidden relative bg-white rounded-lg shadow-md">
                      ...
                      <Image
                        ...
                        height={200}
                        priority
                        src={`/api/images/${prop.clients.image.filename}`}
                        alt={prop.clients.image.name}
